...
export default async function ActivitiesCompleted2(
  ...
  badge,
  preferences,
  noOfActivitiesByGoals
) {
  let goalsDesc = noOfActivitiesByGoals.map(
    (noOfActivities) => noOfActivities.goal
  );
  const colours = [
    "#FBF7F8",
    "#FFC3DD",
      ...
      datasets: [
        {
          ...
          data: noOfActivitiesByGoals.map(
            (noOfActivities) => noOfActivities.count
          ),
        ...
        value: `
            **Health Areas**
            ${
              goalsDesc.length === 0
                ? "No Goals Set"
                : goalsDesc.map((goal) => `🎯 ${goal}`).join("\n")
            }
            
            **Frequency**
            ${
              preferences.frequency === 0
                ? "Frequency Not Set"
                : `Every ${preferences.frequency} minutes`
            }
            `,
      }
    )
